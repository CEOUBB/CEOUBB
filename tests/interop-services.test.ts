...
test("REQ-IO-01–11 servicios, contratos HTTP y migración sobre libSQL", async (t) => {
  ...
  await t.test("lista con cursor y límite institucional", async () => {
    await assert.rejects(
      () => listInteropResources(student, section, "invalid cursor!"),
      status(400)
    );
  ...
  await t.test(
    "todas las consultas select en interop.ts llevan un .limit() explícito",
    async () => {
      const source = await readFile(new URL("../lib/services/interop.ts", import.meta.url), "utf8");
      ...
      for (const query of queries) {
        ...
        assert.ok(
          ...
          `se detectó una consulta sin límite: .select(${statement}`
        );
      }
    }
  );
