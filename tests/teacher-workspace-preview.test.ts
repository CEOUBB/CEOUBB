...
test("REQ-DOC-02 y REQ-DOC-13 crean fixtures sintéticos nuevos en cada carga", () => {
  ...
  assert.ok(
    second.submissions.every((submission) => /^Estudiante \d{2}$/.test(submission.studentAlias))
  );
...
test("REQ-DOC-05 conserva el contexto y valida borrador, fechas y ponderación", () => {
  ...
  assert.deepEqual(Object.keys(validateActivity(activity, "draft")).sort(), [
    "instructions",
    "title",
  ]);
  const invalid = {
    ...
    opensAt: "2026-08-20T10:00",
    dueAt: "2026-08-19T10:00",
    cutoffAt: "2026-08-18T10:00",
    gradeWeight: 120,
  };
  ...
  const valid = {
    ...invalid,
    dueAt: "2026-08-21T10:00",
    cutoffAt: "2026-08-22T10:00",
    gradeWeight: 20,
...
test("REQ-DOC-07 mantiene contadores coherentes y ordena el trabajo por urgencia", () => {
  ...
  assert.deepEqual(
    work,
    [...work].sort((left, right) => Date.parse(left.dueAt) - Date.parse(right.dueAt))
  );
});
...
test("REQ-DOC-08 filtra y pagina la cola de manera estable", () => {
  ...
  assert.deepEqual(
    paginateSubmissions(state.submissions, "control-1", "all", "Estudiante 01", 1).items.map(
      (item) => item.studentAlias
    ),
    ["Estudiante 01"]
  );
});
...
test("REQ-DOC-09 mantiene privada una corrección guardada como borrador", () => {
  ...
  const state = teacherPreviewReducer(initial, {
    type: "save_review_draft",
    submissionId: "sub-01",
    grade: 5.4,
    feedback: "Buen desarrollo",
    rubric: { planteamiento: 1.8, desarrollo: 2, comunicacion: 1.6 },
  });
...
test("REQ-DOC-10 publica nota y feedback juntos con historial inmutable", () => {
  const initial = createInitialTeacherPreviewState();
  const draft = teacherPreviewReducer(initial, {
    type: "save_review_draft",
    submissionId: "sub-01",
    grade: 5.4,
    feedback: "Buen desarrollo",
  ...
  const published = teacherPreviewReducer(draft, {
    type: "publish_review",
    submissionId: "sub-01",
    grade: 5.6,
    feedback: "Buen desarrollo; revisa las unidades.",
    rubric: { planteamiento: 1.8, desarrollo: 2.1, comunicacion: 1.7 },
  });
...
test("el libro usa solamente calificaciones publicadas", () => {
  ...
  const draft = teacherPreviewReducer(initial, {
    ...
    grade: 7,
    feedback: "Privada",
    rubric: { planteamiento: 2.2, desarrollo: 2.2, comunicacion: 2.1 },
  });
...
test("la ruta no importa backend ni ofrece controles de entrega estudiantil", async () => {
  ...
  assert.doesNotMatch(
    source,
    /firebase-classroom-client|firebase-client|@libsql|TURSO_DATABASE|localStorage|sessionStorage|indexedDB/i
  );
