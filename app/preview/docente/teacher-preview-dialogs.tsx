...
export function ActivityEditorDialog({ activity, onClose, onSave }: ActivityEditorDialogProps) {
  ...
  return (
    <dialog className={`planner-dialog ${styles.editorDialog}`} ref={dialogRef} onClose={onClose} onCancel={onClose} aria-labelledby="activity-dialog-title">
      <form ref={formRef} onSubmit={onSubmit} noValidate>
        <header className={styles.dialogHead}>
          <div>
            <h2 id="activity-dialog-title">{activity.title ? "Editar actividad" : "Nueva actividad"}</h2>
            <p>Actividad {draft.lifecycle === "draft" ? "en borrador" : ACTIVITY_LIFECYCLE_LABELS[draft.lifecycle].toLocaleLowerCase("es-CL")} · sección MCI-210</p>
          </div>
          <button type="button" aria-label="Cerrar editor" onClick={onClose}><X size={18} aria-hidden="true" /></button>
        </header>

        <div className={styles.dialogBody}>
          <div className={styles.formBlock}>
            <div className="section-title compact-title"><h2>Lo esencial</h2></div>
            <label htmlFor="activity-title">
              Título
              <input id="activity-title" value={draft.title} onChange={(event) => update("title", event.target.value)} aria-invalid={Boolean(errors.title)} aria-describedby={errors.title ? "activity-title-error" : undefined} />
            </label>
            ...
            <label htmlFor="activity-instructions">
              Instrucciones
              <textarea id="activity-instructions" rows={5} value={draft.instructions} onChange={(event) => update("instructions", event.target.value)} aria-invalid={Boolean(errors.instructions)} aria-describedby={errors.instructions ? "activity-instructions-error" : undefined} />
            </label>
            ...
            <div className={styles.formGrid}>
              <label htmlFor="activity-unit">
                Unidad
                <input id="activity-unit" value={draft.unit} onChange={(event) => update("unit", event.target.value)} />
              </label>
              <label htmlFor="activity-outcome">
                Resultado de aprendizaje
                <input id="activity-outcome" value={draft.learningOutcome} onChange={(event) => update("learningOutcome", event.target.value)} />
              </label>
            </div>
          </div>

          <div className={styles.formBlock}>
            <div className="section-title compact-title"><h2>Calendario y evaluación</h2></div>
            <p className="grades-note">Una sola definición alimenta la actividad, la agenda y el libro de notas.</p>
            <div className={styles.dateGrid}>
              <label htmlFor="activity-opens">
                Apertura
                <input id="activity-opens" type="datetime-local" value={draft.opensAt} onChange={(event) => update("opensAt", event.target.value)} aria-invalid={Boolean(errors.opensAt)} aria-describedby={errors.opensAt ? "activity-opens-error" : undefined} />
              </label>
              <label htmlFor="activity-due">
                Vencimiento
                <input id="activity-due" type="datetime-local" value={draft.dueAt} onChange={(event) => update("dueAt", event.target.value)} aria-invalid={Boolean(errors.dueAt)} aria-describedby={errors.dueAt ? "activity-due-error" : undefined} />
              </label>
              <label htmlFor="activity-cutoff">
                Cierre
                <input id="activity-cutoff" type="datetime-local" value={draft.cutoffAt} onChange={(event) => update("cutoffAt", event.target.value)} aria-invalid={Boolean(errors.cutoffAt)} aria-describedby={errors.cutoffAt ? "activity-cutoff-error" : undefined} />
              </label>
            </div>
            ...
            <div className={styles.formGrid}>
              <label htmlFor="activity-grade-item">
                Vinculación de nota
                <select id="activity-grade-item" value={draft.gradeItemId ? "graded" : "none"} onChange={(event) => update("gradeItemId", event.target.value === "graded" ? draft.id : null)}>
              ...
              <label className={styles.suffixField} htmlFor="activity-weight">
                Ponderación
                <input id="activity-weight" type="number" min="0" max="100" value={draft.gradeWeight} onChange={(event) => update("gradeWeight", finiteInputValue(event.currentTarget.valueAsNumber, 0))} aria-invalid={Boolean(errors.gradeWeight)} aria-describedby={errors.gradeWeight ? "activity-weight-error" : undefined} disabled={!draft.gradeItemId} />
          ...
          <details className={styles.advanced}>
            <summary>
              <span><strong>Configuración de entrega</strong><small>Intentos, formato y archivos aceptados</small></span>
            ...
            <div className={styles.advancedBody}>
              <div className={styles.formGrid}>
                <label htmlFor="activity-mode">
                  Modalidad
                  <select id="activity-mode" value={draft.submissionMode} onChange={(event) => update("submissionMode", event.target.value as SubmissionMode)}>
                ...
                <label htmlFor="activity-attempts">
                  Intentos máximos
                  <input id="activity-attempts" type="number" min="1" max="10" value={draft.maxAttempts} onChange={(event) => update("maxAttempts", Math.max(1, finiteInputValue(event.currentTarget.valueAsNumber, 1)))} />
                </label>
              </div>
...
export function StudentPreviewDialog({ state, submissionId, onClose }: StudentPreviewDialogProps) {
  ...
  return (
    <dialog className={`planner-dialog ${styles.studentDialog}`} ref={dialogRef} onClose={onClose} onCancel={onClose} aria-labelledby="student-preview-title">
      <header className={styles.dialogHead}>
        <div>
          <h2 id="student-preview-title">Así lo verá el estudiante</h2>
          <p>Sólo lectura · nada de esto se guarda</p>
        </div>
        <button type="button" aria-label="Cerrar Vista estudiante" onClick={onClose}><X size={18} aria-hidden="true" /></button>
      </header>
            ...
            <div>
              {visibleActivities.map((item) => (
                <div className={styles.studentRow} key={item.id}>
                  <span className={styles.studentIcon} aria-hidden="true"><FileText size={19} /></span>
          ...
          {activity && (
            <section className={styles.card} aria-labelledby="student-result-title">
              <div className="section-title compact-title">
                ...
                <span className="grades-note">{submission?.studentAlias ?? "Estudiante ficticio"}</span>
              </div>
              {review ? (
      ...
      <footer className={styles.dialogFoot}>
        <span><Eye size={16} aria-hidden="true" /> Vista de sólo lectura del aula del estudiante.</span>
        <div>
          <button className="secondary-button" type="button" onClick={onClose}>Volver al espacio docente</button>
        </div>
      </footer>
    </dialog>
  );
