  ...
  return (
    <m.article
      ...
      exit={
        shouldReduceMotion
          ? { opacity: 0 }
          : { opacity: 0, transform: "scale(0.96)", transition: { duration: 0.12 } }
      }
      initial={shouldReduceMotion ? false : { opacity: 0, transform: "scale(0.94)" }}
      key={block.id}
      style={
        {
      ...
      transition={shouldReduceMotion ? instantTransition : { duration: 0.2, ease }}
    >
      <button
        aria-label={
          block.completed
