...
- El portal resuelve Firebase por entorno con configuración completa o fallo cerrado. Vercel Preview usa exclusivamente los identificadores y la base staging mediante el alias OAuth autorizado `ceoubb-staging.vercel.app`, mientras Production conserva sus valores anteriores.
...
- DONE — Codex / Juako, `elpapijuaco325/ceo-74-generacion-y-certificacion-oficial-de-actas-de-notas-finales`: **CEO-74 — Generación y certificación docente de actas finales DUE 5420/2023 (2026-08-29).** PR [#96](https://github.com/CEOUBB/CEOUBB/pull/96) (commit `ebef192`). Implementa el cálculo institucional 60/40 con `round1`, condición obligatoria o voluntaria de evaluación integradora, resumen de aprobados/reprobados/pendientes, nómina activa completa y acotada desde Turso, escritura de integradoras por la mutación auditada y exportaciones locales XLSX/PDF con identidad docente, instante y huella SHA-256. El libro incluye `Acta final`, `Carga Intranet`, `Detalle evaluaciones` y `Resumen`; ambos artefactos preservan el descargo de independencia y no se presentan como sustituto jurídico del acta DARCA. Verificación: focal 22/22, `verify:fast` 440/440 y 25/25 specs, invariantes 33/33, build Next.js 16.3 + suite integral 465/465, typecheck, lint y formato limpios, QA estructural y visual de XLSX/PDF y React Doctor 98/100 sin hallazgos nuevos. Especificación: `docs/specs/p19-final-grade-records.md` (`VERIFICADA`).
...
- DONE — Antigravity, `fix_ceo17_linear_issue`: **CEO-17 — Paginación de listas grandes y partición de notas (2026-08-27).** PR [#87](https://github.com/CEOUBB/CEOUBB/pull/87) (commit `1b9c065`). Paginación en cliente para cursos masivos (>300 estudiantes) en libro de notas (`TeacherGrades`), nómina docente de avance (`ProgressSection`), y filtrado insensible a acentos/mayúsculas en materiales y participantes. Verificación de invariantes de partición en lotes $\le 400$ operaciones en `saveSectionScores` y `chunkOperations`. Verificación: `verify:fast` (405/405 tests pasados en <3.0s), invariantes (32/32 tests), build de producción Next.js 16.3 + suite integral (430/430 tests), typecheck, lint, formato Prettier, OpenSpec estricto y React Doctor 0 issues en el ámbito modificado. OpenSpec: `paginate-large-course-lists`.
- DONE — Antigravity, `feat/perf-optimizations`: **CEO-PERF — Optimizaciones de rendimiento full-stack y escalabilidad del sistema (2026-08-26).** PR [#86](https://github.com/CEOUBB/CEOUBB/pull/86) (commit `f072bc3`). Implementa optimizaciones de persistencia relacional en Turso (eliminación de `upper`/`lower` en filtros indexados, multi-row batch inserts en importación de Moodle, índice compuesto en `matriculas(usuario_id, estado)` con migración `0009`, deduplicación de consultas en `/api/enrollments/me`), optimizaciones de frontend (eliminación de hashing FNV-1a y JSON.stringify recursivo en `RichText.tsx`, aislamiento de estado de edición en `PostsSection.tsx`), Next.js bundle & API (optimización de package imports para `@phosphor-icons/react` en `next.config.ts`, paralelización de llamadas GitHub en `pr-reviewer.ts`, catálogo docente condicional al cursor en `/api/teacher/courses`), y assets móviles (poda de fuentes Merriweather y desacoplamiento de KaTeX global en `layout.tsx`, alineación de privacidad en Sentry). Verificación: `verify:fast` 396/396, invariantes 32/32, build Next.js 16.3 + suite integral 421/421, `lint` y `format:check` limpios, React Doctor sin nuevos hallazgos. OpenSpec: `optimize-system-performance`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-12-crear-un-ambiente-de-pruebas-separado-de-produccion`: **CEO-12 — Ambiente staging separado de producción (2026-08-23).** PR [#78](https://github.com/CEOUBB/CEOUBB/pull/78) (commit `1835e25`). Firebase `centro-de-estudio-ubb-staging` en `southamerica-west1`, Turso `ceoubb-staging`, resolución estricta por entorno (`staging` vs `production`), sembrado sintético idempotente con 5 facultades, 5 programas, 5 secciones, 20 usuarios y 32 notas. Verificación: `verify:fast` 255/255, invariantes 31/31, build + suite integral 280/280, smoke test y OpenSpec estricto archivado. Capacidad viva: [`operations/staging`](../../openspec/specs/operations/staging/spec.md); cambio archivado: [`2026-08-23-create-staging-environment`](../../openspec/changes/archive/2026-08-23-create-staging-environment/proposal.md).
- DONE — Codex / Juako, `elpapijuaco325/ceo-39-poder-importar-cursos-que-ya-existen-en-moodle-ubb`: **CEO-39 — Importación de cursos y respaldos Moodle UBB (2026-08-23).** PR [#66](https://github.com/CEOUBB/CEOUBB/pull/66) (commit `635d035`). Implementa análisis local acotado de `.mbz` TGZ/ZIP y nóminas CSV; previsualización; conversión segura de secciones, `page`, `label`, `url`, `assign`, `book`, `resource`, `folder` y paquetes SCORM descargables; restauración idempotente por IDs deterministas; cargas Storage con verificación SHA-1; API autorizada por matrícula docente; nómina estudiantil existente o pendiente por 90 días; bitácora Turso y reporte JSON con omisiones explícitas. Migración `drizzle/0007_purple_tyger.sql`. Verificación: focal 10/10; `verify:fast` 237/237; invariantes 31/31; build y suite integral 262/262; typecheck, lint, formato, Functions y OpenSpec estricto limpios. Capacidad viva: [`integrations/moodle-course-import`](../../openspec/specs/integrations/moodle-course-import/spec.md); cambio archivado: [`2026-08-23-import-moodle-course-backups`](../../openspec/changes/archive/2026-08-23-import-moodle-course-backups/proposal.md).
- DONE — Codex / Juako, `elpapijuaco325/ceo-18-cierre-de-semestre-archivar-ramos-del-periodo-anterior`: **CEO-18 — Cierre de semestre y archivo de secciones (2026-08-24).** PR [#76](https://github.com/CEOUBB/CEOUBB/pull/76) (commit `46e687a`). El owner puede archivar un período sin borrar secciones, matrículas ni aula; la proyección estrecha sección-período bloquea mutaciones en Firestore, Storage y Functions, mientras Web/Capacitor separa el historial paginado y muestra el aula en solo lectura. Repetir asignatura conserva dos identidades de sección y matrícula. Verificación: prueba focal 6/6, `verify:fast` 254/254, 14 specs, lint/Functions limpios, React Doctor 89/100, navegador escritorio/móvil sin overlay y `pnpm test` con build Next.js 16.3 y 279/279. Spec: `docs/specs/p18-semester-archival.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-21-que-el-profe-pueda-dejar-un-comentario-en-cada-nota`: **CEO-21 — Retroalimentación privada por nota (2026-08-23).** PR [#73](https://github.com/CEOUBB/CEOUBB/pull/73) (commit `5da4f6e`). Cada nota oficial admite un comentario privado de hasta 2.000 caracteres; una Callable dedicada valida docente y matrícula, exige la nota previa y escribe comentario más auditoría en una transacción. Vaciar la nota también retira y audita su comentario. El estudiante lo lee sólo desde su documento en tabla web y hoja móvil; la matriz docente monta un único editor accesible, sin nuevas consultas, listeners, índices ni dependencias. Verificación: contrato 7/7, `verify:fast` 255/255, invariantes 31/31, build y suite integral 280/280, lint/typecheck/formato/Functions limpios, React Doctor changed-scope sin hallazgos y OpenSpec estricto archivado. Spec viva: [`grades`](../../openspec/specs/grades/spec.md); cambio archivado: [`2026-08-23-add-private-grade-feedback`](../../openspec/changes/archive/2026-08-23-add-private-grade-feedback/proposal.md).
- DONE — Codex / Juako, `elpapijuaco325/ceo-24-mejorar-la-lista-de-participantes-buscador-roles-y-contacto`: **CEO-24 — Directorio de participantes (2026-08-23).** PR [#74](https://github.com/CEOUBB/CEOUBB/pull/74) (commit `58be04b`). Padrón Turso autorizado por matrícula, búsqueda y filtros servidor, cursor estable, separación equipo docente/ayudantes/estudiantes, contacto individual y estados accesibles en Web/Capacitor. QA con 304 matrículas: búsqueda, filtros, grupos, correo, carga 24 → 48, móvil 390 × 844 sin overflow y Axe 0/0. Verificación: `verify:fast` 253/253, invariantes 31/31, formato/lint/Functions limpios y `pnpm test` con build Next.js 16.3 + 278/278. Spec: `docs/specs/p18-participant-directory.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-26-centro-de-avisos-y-mensajes-dentro-de-la-plataforma`: **CEO-26 — Centro de avisos y mensajes dentro de la plataforma (2026-08-23).** PR [#75](https://github.com/CEOUBB/CEOUBB/pull/75) (commit `a8a7b44`). Avisos recientes de publicaciones matriculadas, contador y cursores leídos persistentes; un hilo privado por estudiante/sección con respuestas de docentes/coordinación, mensajes inmutables y autor/reloj verificados. Límites: 40 secciones, 25 hilos por sección, 100 mensajes, 200 cursores y 120 resultados combinados. Verificación: focal 9/9, `verify:fast` 257/257, invariantes 31/31, build Next.js 16.3 y suite integral 282/282, lint/formato/Functions limpios, React Doctor sin hallazgos y navegador 1280/390/320 px sin errores ni overflow. Spec viva: [`communications/center`](../../openspec/specs/communications/center/spec.md); cambio archivado: [`2026-08-23-add-communications-center`](../../openspec/changes/archive/2026-08-23-add-communications-center/proposal.md).
- DONE — Codex / Juako, `elpapijuaco325/ceo-47-activar-app-check-para-que-solo-nuestras-apps-usen-el`: **CEO-47 — App Check para Web y Android (2026-08-23).** PR [#77](https://github.com/CEOUBB/CEOUBB/pull/77) (commit `80926e9`). reCAPTCHA Enterprise protege el portal y la WebView remota; Play Integrity se instala antes de Firebase Auth nativo. Las APIs y ambos proveedores quedaron registrados con TTL de una hora; Firestore, Storage y Authentication siguen `UNENFORCED`, y las callable Functions declaran observación explícita. El runbook exige 24 horas, ≥99 % de tokens válidos y enforcement escalonado con reversa. Verificación: focal 6/6, `verify:fast` 254/254, invariantes 31/31, build Next.js + suite integral 279/279, Android `assembleDebug` 197/197, lint/formato/Functions limpios. Spec: `docs/specs/p18-firebase-app-check-rollout.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-42-auditar-la-accesibilidad-del-portal-y-la-biblioteca`: **CEO-42 — Auditoría y declaración WCAG 2.2 AA (2026-08-23).** PR [#65](https://github.com/CEOUBB/CEOUBB/pull/65) (commit `7871020`). Portal, vista docente, biblioteca y documentos públicos incorporan bypass, foco contextual, nombres/roles/estados dinámicos, formularios asociados, objetivos de 24 px, contraste de texto y controles, movimiento reducido y reflujo sin desplazamiento horizontal a 320 px. `/accesibilidad` publica alcance, fecha, tecnologías, método, compatibilidad, contacto, exclusiones y el descargo independiente; sitemap y entradas principales la enlazan. La biblioteca versiona CSS/JS y eleva el service worker a `v8` para invalidar la caché anterior. Auditoría de navegador: seis superficies a 320/640 CSS px, árbol accesible y consola limpia. Verificación: accesibilidad 7/7, `verify:fast` 234/234, invariantes 31/31, formato/lint/Functions limpios y `pnpm test` 259/259. Spec: `docs/specs/p17-wcag-22-aa-conformance.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-25-agregar-el-rol-de-ayudante`: **CEO-25 — Rol de ayudante por ramo (2026-08-23).** PR [#67](https://github.com/CEOUBB/CEOUBB/pull/67) (commit `99ff3bc`). La sesión transporta matrículas acotadas `{ sectionId, role }`; el portal resuelve el rol de la sección abierta y muestra «Ayudante» sin alterar la identidad global de estudiante. `assistant` puede publicar y administrar únicamente contenido propio; notas, progreso agregado y live class permanecen bajo autorización docente. Firestore y Storage aplican la misma separación sin agregar sentencias `allow`, consultas ni listeners. Verificación: prueba focal 5/5, `verify:fast` 232/232, invariantes 31/31, lint/typecheck/formato limpios, build Next.js 16.3 + suite integral 257/257 y React Doctor changed-scope sin hallazgos. Spec viva: `openspec/specs/academic/spec.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-7-guardar-historial-de-cambios-de-notas-quien-cambio-que-y`: **CEO-7 — Historial inmutable de cambios de notas (2026-08-23).** PR [#70](https://github.com/CEOUBB/CEOUBB/pull/70) (commit `9fd8365`). Notas y ponderaciones se escriben por Functions autenticadas; estado y evidencia con autor, reloj de servidor y valores anterior/nuevo comparten transacción. Las reglas bloquean todo bypass cliente y la lectura estudiantil queda aislada por UID. Lotes: 100 estudiantes por invocación, diez transacciones concurrentes. Verificación: `verify:fast` 236/236, invariantes 31/31, build y suite integral 261/261, lint/formato/Functions limpios.
- DONE — Codex / Juako, `elpapijuaco325/ceo-40-preparar-la-carpeta-legal-convenio-retencion-y-borrado-de`: **CEO-40 — Carpeta legal: convenio, retención y borrado de datos (2026-08-23).** PR [#69](https://github.com/CEOUBB/CEOUBB/pull/69) (commit `46fde50`). `docs/legal/` separa el rol contractual futuro (UBB responsable, operador legal de CEOUBB encargado) de la condición independiente actual; propone tabla de retención y supresión verificable, canal de derechos sin WhatsApp, registro honesto de residencia/subencargados y plan de salida.
- DONE — Codex / Juako, `elpapijuaco325/ceo-27-panel-del-docente-para-administrar-su-ramo`: **CEO-27 — Panel docente para administrar su ramo (2026-08-23).** PR [#64](https://github.com/CEOUBB/CEOUBB/pull/64) (commit `56ffb67`). El docente y el owner disponen de `Administrar ramos` para crear una sección con catálogo académico, editar su ficha, configurar evaluaciones que suman exactamente 100% y designar o retirar ayudantes institucionales sin perder su matrícula previa. Turso crea sección, perfil y matrícula mediante batch atómico; una falla de proyección Firestore compensa el cambio. El portal dejó el catálogo estático y carga sólo matrículas activas; todas las listas se acotan a 100 y la evaluación seleccionada abre un único listener documental. Verificación: focal CEO-27 9/9, `verify:fast` 236/236, invariantes 31/31, build + suite integral 261/261, lint/formato/Functions limpios, React Doctor sin hallazgos. Especificación verificada: `docs/specs/p17-teacher-course-management.md`; OpenSpec archivado como `2026-08-23-add-teacher-course-management`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-16-poder-cargar-matriculas-masivamente-desde-una-planilla`: **CEO-16 — Carga masiva de matrículas desde CSV (2026-08-23).** PR [#68](https://github.com/CEOUBB/CEOUBB/pull/68) (commit `1e9df38`). Implementación y spec `VERIFICADA`: previsualización sin mutaciones, autorización por sección, batches atómicos e idempotentes en Turso, pendientes por correo para cuentas sin primer ingreso, proyección Firestore por lotes e interfaz docente paginada. Verificación: focal 11/11 con batch de más de 100 filas, fast 239/239, invariantes 32/32, build y suite completa 264/264, React Doctor changed-scope 90/100 sin hallazgos propios y QA web/móvil sin overflow ni errores. Spec: `docs/specs/p17-bulk-enrollment-import.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-59-feateditor-modo-multimodal-en-tres-pestanas`: **CEO-59 — Editor académico en tres pestañas (2026-08-20).** PR [#59](https://github.com/CEOUBB/CEOUBB/pull/59) (commit `8b90aa5`). `MultimodalEditor` integra modos Visual, Markdown + LaTeX y HTML libre sobre el contrato controlado `body: string`; el conversor conserva como HTML recuperable aquello que Markdown no representa y la proyección visual descarta nodos ejecutables sin `dangerouslySetInnerHTML`. La barra ofrece negrita, cursiva, subrayado, alineaciones, tabla, fórmula, código, callout y enlace; incluye atajos `Ctrl/Cmd+B/I/K`, pestañas automáticas, foco itinerante, anuncios accesibles y controles táctiles de 44 px. Archivos principales: `app/views/classroom/MultimodalEditor.tsx`, `lib/multimodal-editor.ts`, `app/globals.css` y `tests/multimodal-editor.test.ts`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-57-feateditor-blindaje-de-diseno-institucional-y-sanitizacion`: **CEO-57 — Blindaje institucional y sanitización de contenido académico (2026-08-20).** PR [#56](https://github.com/CEOUBB/CEOUBB/pull/56) (commit `6044074`). `sanitizeAcademicHtml` usa `isomorphic-dompurify@3.0.0` con allowlist por elemento, elimina scripts/eventos/estilos Word-Moodle/custom elements, falla cerrado ante protocolos y credenciales, limita imágenes a HTTPS/internas y refuerza enlaces externos con `noopener noreferrer`. `AcademicProse` es el único sink React; `.academic-prose` consume Manrope/Merriweather, tokens CEOUBB, código desplazable e imágenes acotadas. Las tablas se envuelven de forma idempotente en una región enfocable con scroll horizontal. Spec: `docs/specs/p15-academic-prose-sanitization.md`; capacidad viva: `openspec/specs/editor/academic-content/spec.md`; cambio archivado: `2026-08-20-add-academic-prose-sanitization`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-60-feataula-asistente-de-publicacion-y-split-button-para-profesores`: **CEO-60 — Asistente de publicación y split-button (2026-08-20).** PR [#58](https://github.com/CEOUBB/CEOUBB/pull/58) (commit `c1a09a6`). Implementa acceso principal síncrono desde `ceoubb_default_editor`, wizard accesible de tipo/modo/destino, menú de cambio inmediato, preferencia local tolerante a fallos y publicación silenciosa retrocompatible. `notifyStudents: false` corta FCM; el campo ausente conserva la conducta histórica. No añade consultas, listeners, dependencias ni escrituras por estudiante. Specs: `docs/specs/p15-publication-wizard.md`, capacidad `openspec/specs/classroom/publication-wizard/` y archivo `openspec/changes/archive/2026-08-20-add-publication-wizard/`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-61-testclassroom-retrocompatibilidad-de-publicaciones-seguridad`: **CEO-61 — Retrocompatibilidad, seguridad y paridad móvil de publicaciones (2026-08-20).** PR [#57](https://github.com/CEOUBB/CEOUBB/pull/57) (commit `959b7bc`). Avisos históricos conservan CRLF y texto dentro de `.academic-prose`; tablas Markdown de seis columnas se construyen con nodos React semánticos seguros; tabla, código y KaTeX confinan overflow horizontal táctil sin ampliar el feed en Web/Capacitor. Sin dependencias, consultas, migraciones ni renderer Android paralelo. Spec: `docs/specs/p16-classroom-retrocompatibility-mobile-parity.md`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-55-editor-rico-para-publicaciones-markdown-bloques-de-codigo-y`: **CEO-55 — Publicaciones técnicas con Markdown, bloques de código y fórmulas (2026-08-20).** PR [#52](https://github.com/CEOUBB/CEOUBB/pull/52) (commit `31c07f2`). Conserva `body: string`, reutiliza KaTeX vendorizado, no añade dependencias ni implementación Android paralela, limita nuevas escrituras a 40.000 caracteres y no agrega consultas. `RichText` construye nodos React escapados sin `dangerouslySetInnerHTML`; KaTeX opera con `trust: false`, límites de expansión/tamaño y fallback al TeX fuente. Creación y edición comparten `RichPostEditor` con vista previa diferida; texto plano histórico sigue visible sin migración. Verificación: `verify:fast` 189/189, `verify:invariants` 31/31, ESLint y Prettier limpios, OpenSpec válido, test-locking 23 archivos y `pnpm test` 214/214 tras build Next.js 16. Espec viva: `classroom/rich-posts`; cambio archivado: `2026-08-20-add-rich-classroom-posts`.
- DONE — Codex / Juako, `elpapijuaco325/ceo-56-banner-de-clase-en-vivo-zoom-teams-en-la-portada-del-ramo`: **CEO-56 — Banner de clase en vivo Zoom/Teams (2026-08-20).** PR [#53](https://github.com/CEOUBB/CEOUBB/pull/53) (commit `5621744`). Contrato HTTPS limitado a Zoom/Teams, documento fijo `meta/live-class`, listener O(1) por aula abierta, edición del equipo docente actual, eliminación en tiempo real y banner responsive antes de avisos sin reservar espacio al estudiante. Las reglas preservan matrícula, `teachesSection` y las 21 sentencias `allow`; ayudantías quedan fuera hasta que exista el rol de sección `assistant`. Verificación: `verify:fast` 191/191, invariantes 31/31, lint/typecheck/Functions limpios, build + suite total 216/216, React Doctor changed-scope sin hallazgos y navegador 1280/390 px sin overflow ni errores. Specs: `docs/specs/p8-live-class-banner.md` y capacidad OpenSpec `classroom/live-class`.
- DONE — Claude Code, `claude/ceo-11-issue-98568f`: **P14 — Privacidad y términos para datos académicos reales (CEO-11, 2026-08-20).** PR [#49](https://github.com/CEOUBB/CEOUBB/pull/49) (commit `260c258`). Cambio OpenSpec `expand-privacy-for-academic-data`, capacidad `legal/privacy-and-terms`: inventario de datos académicos en `app/privacidad/page.tsx`, divulgación de destinatarios para rol owner, marco legal bajo Ley 21.719, retención de IP de auditoría por 12 meses (`AUDIT_IP_RETENTION_MONTHS = 12`) y purga en `app/api/cron/audit-retention/route.ts`, derechos del titular con canal `contacto@ceoubb.com`, términos de uso en `app/terminos/page.tsx`, fijación de telemetría Sentry (`maskAllText`, `maskAllInputs`, `blockAllMedia`). Verificación: `verify:fast` (169/169), `verify:invariants` (31/31), `lint` (0/0), `pnpm test` (194/194).
- DONE — Antigravity, `fix/deep-security-and-isolation-remediation`: **P13 — Remediación de Seguridad en Profundidad, Aislamiento de Datos y Blindaje de Invariantes (2026-08-19).** PR [#42](https://github.com/CEOUBB/CEOUBB/pull/42) (commit `56e4575`). Spec `docs/specs/p13-deep-application-security-and-isolation-remediation.md` (`VERIFICADA`): normalización de UID en proyección de matrículas, protección IDOR en entregas de evaluaciones, blindaje contra inyección de comandos en Discord bridge, sincronización dual-store de roles en el servidor (`PATCH /api/admin/users`), aislamiento de diagnóstico Sentry (`/api/sentry-test`), saneamiento de protocolos de enlaces (`http://`, `https://`), protección de cuenta propietaria en `DELETE /api/auth/me`, higiene en webhooks y firmas criptográficas (`lib/linear-signature.ts`). Verificación: `verify:fast` (157/157), `verify:invariants` (31/31), `lint` limpio, `pnpm test` (182/182).
- DONE — Claude Code, `feat/ceo-deliberate-ui-ux-remediation`: **P11 — Remediación UI/UX Deliberate, SSOT Tipográfica Manrope/Merriweather y Rendimiento de Movimiento (2026-08-19).** PR [#40](https://github.com/CEOUBB/CEOUBB/pull/40) (commit `d90fec9`). Spec `docs/specs/p11-ui-ux-deliberate-remediation.md` (`VERIFICADA`): SSOT tipográfica Merriweather/Manrope en `DESIGN.md`, `AGENTS.md`, etc.; limpieza de kickers/eyebrows; numerales tabulares `.num`; accesibilidad `:focus-visible` y `:active` inmediato; rendimiento de movimiento con transforms GPU (`translateY`, `scaleX`) y `useReducedMotion()`; calibración de tokens y colores institucionales. Verificación: `verify:fast` (146/146), `verify:invariants` (31/31), `typecheck`, `lint` y `deliberate check` limpios.
- DONE — Pipe / Antigravity / Claude Code, `feat/nucleo-academico-institucional`: **P10 — Núcleo académico institucional, aislamiento por matrícula y tiempo real acotado (2026-08-17).** PR [#35](https://github.com/CEOUBB/CEOUBB/pull/35) (commit `4c45789`). Spec `docs/specs/p10-institutional-lms-evolution.md`: modelo de datos relacional en Turso (`db/schema.ts`, migración `0004_dear_lorna_dane.sql`), eliminación de `DEVELOPER_EMAILS` personales en favor de `users.role = 'owner'`, proyección de matrículas a `enrollments/{uid}/sections/{seccionId}`, consultas acotadas O(1) por sección en `watchCourseActivity` y `watchGradebooks`, partición de matriz de notas en lotes de 400 (`saveSectionScores`), entregas de evaluaciones con comprobante en `courses/{seccionId}/submissions/{evalId}/{uid}/`. Verificación: `typecheck`, `lint`, `verify:fast` (146/146), `verify:invariants` y `pnpm test`.
- DONE — Antigravity, `feat/react-doctor-100-health-score`: **React Doctor 100 Health Score & Frontend Reliability Remediations (2026-08-17).** PR [#31](https://github.com/CEOUBB/CEOUBB/pull/31) (commit `ac00856`). 100/100 en React Doctor con 0 advertencias y 0 errores: modularización de `app/Portal.tsx` (`portal-screens.tsx`, `portal-shell.tsx`, `portal-sheets.tsx`, `portal-types.ts`), `CalendarView.tsx` (`CalendarHeader.tsx`), migración de layout animations a GPU transforms, unificación de estado atómico con `useReducer`, limpieza de código muerto y exports no utilizados, eliminación de mutaciones de autorización desde cliente, e integración de listeners Capacitor en `useHardwareBack`.
- DONE — Antigravity, `feat/governance-sdd-evolution`: **P9: Evolución del Arnés de Desarrollo, Gobernanza de Contexto y SDD Enterprise (2026-08-17).** PR [#29](https://github.com/CEOUBB/CEOUBB/pull/29) (commit `be70ac7`). Spec `docs/specs/p9-enterprise-harness-evolution.md`: test-locking determinista SHA-256 (`scripts/verify-test-hashes.mjs`), puertas de verificación rápida `verify:fast` (<3s) e invariantes, reglas modulares `.agents/rules/*.mdc`, ADRs formales 0001-0004 en `docs/architecture/adr/`, SDD v4.0.0.
- DONE — Antigravity, `feat/governance-sdd-evolution`: **Dual-store user role mutation sync (2026-08-17).** PR [#29](https://github.com/CEOUBB/CEOUBB/pull/29) (commit `be70ac7`). Resuelve desincronización de roles docentes entre Turso (`users.role`) y Firestore (`users/{uid}.role`) vía `updateRemoteUserRole` y `AdminView.tsx`.
- DONE — Antigravity, `feat/perf-and-scalability-remediations`: **P8: Optimizaciones de Rendimiento, Escalabilidad y Desacoplamiento de Carga (2026-08-16).** PR [#26](https://github.com/CEOUBB/CEOUBB/pull/26) (commit `c836e36`). Spec `docs/specs/p8-performance-and-scalability-remediations.md`: índices en FK `sessions.user_id`, función `pruneExpiredSessions`, paginación y búsqueda en `GET /api/admin/users` y `AdminView`, code-splitting dinámico en `app/Portal.tsx`, mapa O(1) en `CoursesDashboard`, memoización de filas de notas en `GradesSection` y lotes `WriteBatch` en Cloud Functions.
- DONE — Antigravity, `feat/ci-cd-automations`: **Automatización de Releases y Distribución de APK (2026-08-16).** PR [#21](https://github.com/CEOUBB/CEOUBB/pull/21) (commit `d8e2563`).
- DONE — Codex / Joaquín, `codex/teacher-assignment-preview`: **P7 — Espacio docente CEOUBB: preview navegable (2026-08-15).** PR [#15](https://github.com/CEOUBB/CEOUBB/pull/15) (commit `8129596`). Implementó inicio docente, actividades, autoría progresiva, cola paginada, mesa de corrección, libro de calificaciones y `Vista estudiante` read-only con fixtures en memoria en `/preview/docente`, montado sobre el shell real del portal. Spec: [`docs/specs/p7-teacher-workspace-preview.md`](../../docs/specs/p7-teacher-workspace-preview.md).
- DONE — Claude Code, `claude/capacitor-migration`: **P5 — Migración de Arquitectura Móvil a Capacitor (2026-08-15).** PR [#13](https://github.com/CEOUBB/CEOUBB/pull/13) (commit `57482ef`). Reemplazó el WebView nativo por Capacitor 7 runtime remote-first, puente `lib/mobile-bridge.ts`, soporte Google Sign-In nativo + `signInWithCredential`, navegación móvil `app/mobile-shell.tsx` con hojas `vaul`, push notifications FCM en `lib/push-notifications.ts`, y visor de archivos nativo en `lib/native-files.ts`. Spec: [`docs/specs/p5-capacitor-mobile-migration.md`](../../docs/specs/p5-capacitor-mobile-migration.md).
- DONE — Pipe / Antigravity / Claude Code, `feat/nucleo-academico-institucional`: **P1 / P10 — Modelo Académico Relacional, Aislamiento por Matrícula y Retiro de Cuentas Personales (2026-08-17).** PR [#35](https://github.com/CEOUBB/CEOUBB/pull/35) (commit `4c45789`). Identidad canónica de sección (_asignatura $\times$ período $\times$ sección_), aislamiento por regla `isEnrolled()` en Firestore, eliminación de barridos `collectionGroup`, entregas de evaluaciones con comprobante, y eliminación de cuentas personales superuser en favor de `users.role = 'owner'`. Specs: [`docs/specs/p1-academic-model.md`](../../docs/specs/p1-academic-model.md) y [`docs/specs/p10-institutional-lms-evolution.md`](../../docs/specs/p10-institutional-lms-evolution.md).

## 2026-08-26: CEO-63, CEO-64 — /contacto y /faq con formulario de soporte institucional

```text
Date: 2026-08-26
Human maintainer: Felipe Arce
AI assistant: Claude (Opus 5)
Branch / commit: felipearce2004/ceo-63-64-contacto-y-faq / PR #81 (commit 7bbf64a)
...
External services changed: Brevo autenticado sobre subdominio notificaciones.ceoubb.com con DKIM/SPF.
...
Production deployed: no
Known risks: retención de solicitudes 12 meses; migración 0008 manual por metadatos drizzle desalineados.
Next recommended action: configurar variables de entorno de Brevo en Vercel y aplicar migración en producción.
```
...
- **Subdominio de envío:** `notificaciones.ceoubb.com` para no colisionar con los registros de Zoho Mail en el dominio raíz.
- **Retención:** 12 meses con hash con pimienta para la dirección del remitente.
- **Sin rayas largas en el texto publicado:** Regla aplicada en todas las páginas públicas.

## 2026-08-26: CEO-66, CEO-65 — panel de notificaciones en cabecera y configuración de perfil/preferencias

```text
Date: 2026-08-26
Human maintainer: Felipe Arce
AI assistant: Claude (Opus 5)
Branch / commit: felipearce2004/ceo-66-65-notificaciones-y-configuracion / PR #88 (commit ca3da5c)
Goal: Panel de notificaciones en campana del header y pantalla de Configuración en el portal con foto, preferencias de push, reducción de movimiento y gestión de sesiones.
Files changed: app/views/SettingsView.tsx, app/notifications-panel.tsx, lib/communications.ts,
  lib/services/user-preferences.ts, db/schema.ts, drizzle/0010_users_photo_url.sql,
  firebase/firestore.rules, firebase/storage.rules, firebase/functions/index.js,
  tests/user-settings.test.ts, tests/notifications.test.ts
External services changed: ninguno directo; requiere publicar reglas y nuevo índice compuesto de Firestore.
Checks passed: typecheck, lint, verify:fast, verify:invariants, pnpm test (421/421).
Checks not run: prueba física de push en Android con tokens FCM reales.
Production deployed: no
Known risks: publicar índice en Firestore antes de desplegar funciones para evitar FAILED_PRECONDITION.
```
