...
Last verified 2026-08-23 · baseline `e032b52` · repo `https://github.com/CEOUBB/CEOUBB.git` · production `https://ceoubb.com`
...
- [EN REVISIÓN] **CEO-DOCTOR — Resolución Integral de Diagnósticos de React Doctor y Modularización de Componentes (2026-08-28).** Rama `feat/fix-react-doctor-findings`, [PR #93](https://github.com/CEOUBB/CEOUBB/pull/93), owner Antigravity. Resuelve el 100% de diagnósticos, modularizando `app/Portal.tsx` en `usePortalCore`, `portalSessionReducer` y `LoadingScreen`, dividiendo `PrivacyPage` en subcomponentes por sección, implementando `AbortController` y `URL.revokeObjectURL` para seguridad de ciclo de vida, optimizando búsquedas con `RegExp` y concurrencia por lotes. Verificación: `verify:fast` (427/427 tests pasados en <3.0s), invariantes (32/32 tests), build de producción Next.js 16.3 + suite integral (452/452 tests), `format:check` y `lint` 100% limpios, 16/16 checks de CI en verde en GitHub Actions y React Doctor 100/100 (0 errores, 0 advertencias). OpenSpec: `fix-react-doctor-findings`.

- [EN REVISIÓN] **CEO-17 — Paginación de listas grandes y partición de notas (2026-08-27).** Rama `fix_ceo17_linear_issue`, owner Antigravity. Paginación en cliente para cursos masivos (>300 estudiantes) en libro de notas (`TeacherGrades`), nómina docente de avance (`ProgressSection`), y filtrado insensible a acentos/mayúsculas en materiales y participantes. Verificación de invariantes de partición en lotes $\le 400$ operaciones en `saveSectionScores` y `chunkOperations`. Verificación: `verify:fast` (405/405 tests pasados en <3.0s), invariantes (32/32 tests), build de producción Next.js 16.3 + suite integral (430/430 tests), typecheck, lint, formato Prettier, OpenSpec estricto y React Doctor 0 issues en el ámbito modificado. OpenSpec: `paginate-large-course-lists`.

- [EN REVISIÓN] **CEO-PERF — Optimizaciones de rendimiento full-stack y escalabilidad del sistema (2026-08-26).** Rama `feat/perf-optimizations`, owner Antigravity. Implementa optimizaciones de persistencia relacional en Turso (eliminación de `upper`/`lower` en filtros indexados, multi-row batch inserts en importación de Moodle, índice compuesto en `matriculas(usuario_id, estado)` con migración `0009`, deduplicación de consultas en `/api/enrollments/me`), optimizaciones de frontend (eliminación de hashing FNV-1a y JSON.stringify recursivo en `RichText.tsx`, aislamiento de estado de edición en `PostsSection.tsx`), Next.js bundle & API (optimización de package imports para `@phosphor-icons/react` en `next.config.ts`, paralelización de llamadas GitHub en `pr-reviewer.ts`, catálogo docente condicional al cursor en `/api/teacher/courses`), y assets móviles (poda de fuentes Merriweather y desacoplamiento de KaTeX global en `layout.tsx`, alineación de privacidad en Sentry). Verificación: `verify:fast` 396/396, invariantes 32/32, build Next.js 16.3 + suite integral 421/421, `lint` y `format:check` limpios, React Doctor sin nuevos hallazgos. OpenSpec: `optimize-system-performance`.

- [EN REVISIÓN] **CEO-12 — Ambiente staging separado de producción (2026-08-23).** Rama `elpapijuaco325/ceo-12-crear-un-ambiente-de-pruebas-separado-de-produccion`, PR [#78](https://github.com/CEOUBB/CEOUBB/pull/78), owner Codex/Juako. Firebase `centro-de-estudio-ubb-staging` en `southamerica-west1`, Turso `ceoubb-staging`, resolución estricta por entorno (`staging` vs `production`), sembrado sintético idempotente con 5 facultades, 5 programas, 5 secciones, 20 usuarios y 32 notas. Verificación: `verify:fast` 255/255, invariantes 31/31, build + suite integral 280/280, smoke test y OpenSpec estricto archivado. Capacidad viva: [`operations/staging`](openspec/specs/operations/staging/spec.md); cambio archivado: [`2026-08-23-create-staging-environment`](openspec/changes/archive/2026-08-23-create-staging-environment/proposal.md).
- [EN REVISIÓN] **CEO-39 — Importación de cursos y respaldos Moodle UBB (2026-08-23).** Rama `elpapijuaco325/ceo-39-poder-importar-cursos-que-ya-existen-en-moodle-ubb`, owner Codex/Juako; PR [#66](https://github.com/CEOUBB/CEOUBB/pull/66). Implementa análisis local acotado de `.mbz` TGZ/ZIP y nóminas CSV; previsualización; conversión segura de secciones, `page`, `label`, `url`, `assign`, `book`, `resource`, `folder` y paquetes SCORM descargables; restauración idempotente por IDs deterministas; cargas Storage con verificación SHA-1; API autorizada por matrícula docente; nómina estudiantil existente o pendiente por 90 días; bitácora Turso y reporte JSON con omisiones explícitas. Cuestionarios, foros, intentos, calificaciones, entregas, ejecución SCORM y LTI 1.3 quedan fuera de esta entrega para no simular compatibilidad que aún no existe. Migración `drizzle/0007_purple_tyger.sql`; política de privacidad actualizada para correos pendientes. Verificación: focal 10/10; `verify:fast` 237/237; invariantes 31/31; build y suite integral 262/262; typecheck, lint, formato, Functions y OpenSpec estricto limpios; transacción Turso probada con un servicio local; diálogo revisado en 1440×900 y 390×844 sin errores de consola ni overflow. Capacidad viva: [`integrations/moodle-course-import`](openspec/specs/integrations/moodle-course-import/spec.md); cambio archivado: [`2026-08-23-import-moodle-course-backups`](openspec/changes/archive/2026-08-23-import-moodle-course-backups/proposal.md).
