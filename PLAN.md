...
- [EN REVISIÓN] **CEO-60 — Asistente de publicación y split-button (2026-08-20).** Rama `elpapijuaco325/ceo-60-featclassroom-asistente-wizard-de-3-pasos-y-split-button-con`, PR [#58](https://github.com/CEOUBB/CEOUBB/pull/58), owner Codex/Juako. Implementa acceso principal síncrono desde `ceoubb_default_editor`, wizard accesible de tipo/modo/destino, menú de cambio inmediato, preferencia local tolerante a fallos y publicación silenciosa retrocompatible. `notifyStudents: false` corta FCM; el campo ausente conserva la conducta histórica. No añade consultas, listeners, dependencias ni escrituras por estudiante. Verificación: `verify:fast` 202/202, invariantes 31/31, lint, formato, Functions y React Doctor limpios; `pnpm test` compila Next.js 16 y aprueba 227/227; navegador real 1280 px y 390 × 844 px sin overflow ni errores. Sin despliegue: la Function debe publicarse después del merge. Specs: `docs/specs/p15-publication-wizard.md`, capacidad `openspec/specs/classroom/publication-wizard/` y archivo `openspec/changes/archive/2026-08-20-add-publication-wizard/`.
