...
- [ACTIVE] Claude Code · `main`, uncommitted — **institutional polish pass** on top of the Design System rebuild, aimed at the university presentation (Canvas-grade chrome). Changes: (1) typographic pairing — `Source Serif 4` (`--font-display`, next/font, weights 600/700) now signs page titles, section titles, course names, cover headlines, the brand lockup and the large grade/progress figures; `Inter` keeps every operational surface; numerals moved to `lining-nums tabular-nums` (`--num`) and monospace is now reserved for registry codes only. (2) The global app bar became a solid UBB Midnight Navy band (58px, `--header-height`) closed by a 2px heraldic rule (royal blue / sky / gold / red); header controls, avatar and account menu were re-toned for the dark surface. (3) Sidebar densified (40px rows, flat icons, flush active rail, hairline course tiles). (4) Dashboard: removed the "Periodo 2026-2" kicker above the h1 (period moved into the subtitle line), page heads gained a hairline rule, course grid is now `auto-fill minmax(304px, 1fr)`, and the course card was restructured with a `.course-body` wrapper so the colour banner is full-bleed — the banner is a 16:5 image slot that already accepts a teacher-uploaded cover through `--course-image` (upload flow itself not built). (5) Aula: square underlined tabs, richer navy cover with the course-tone rule and the eyebrow demoted to a chip, redundant `01/02` unit numbering removed. (6) Calendar aside no longer vertically centred; timeline, resources and empty states re-toned. (7) Access screen moved off its off-palette blues onto DS tokens with a white form card. `design-ceoubb.md` updated (serif ramp, `app-bar`, `course-card-banner`, new Do/Don't rules). Verified: `pnpm test` 44/44, `pnpm run lint` clean for `app/`, impeccable design detector clean except the deliberate `.access-brand::after` heraldic edge stripe, computed-style audit against the signed-in dashboard (navy `rgb(0,43,92)` bar, serif h1 38px, no horizontal overflow at 1905px). **Not verified:** signed-in screenshots (the in-app browser pane could not composite frames) and the aula/notas/participantes tabs behind the login — a manual visual pass is still owed. `app/globals.css`, `app/layout.tsx`, `app/portal-views.tsx`, `app/Classroom.tsx`, `design-ceoubb.md`, `.claude/launch.json`
...
- [ACTIVE] Codex · `main`, uncommitted — portal polish: the upcoming-evaluation strip now offers `Ir al ramo`; unavailable header controls and the course-page role badge were removed; sidebar navigation remains open; course rows use tone-coloured folders; course cards use colour-only banners; course reference (`código - sección`) copies from the classroom header; empty announcement states direct teachers to publish and students to the course library. `pnpm run lint`, `pnpm test`, and the Impeccable detector ran locally; the signed-in browser pass remains blocked on institutional authentication. `app/Portal.tsx`, `app/Classroom.tsx`, `app/portal-views.tsx`, `app/globals.css`, `lib/courses.ts`
...
- [BACKLOG] Calendar month and week views, recurring weekly class schedules. `app/portal-views.tsx`
