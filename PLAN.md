...
- [EN REVISIÓN] **CEO-55 — Publicaciones técnicas con Markdown, bloques de código y fórmulas (2026-08-20).** Rama `elpapijuaco325/ceo-55-editor-rico-para-publicaciones-markdown-bloques-de-codigo-y`; PR [#52](https://github.com/CEOUBB/CEOUBB/pull/52); especificación viva [`classroom/rich-posts`](openspec/specs/classroom/rich-posts/spec.md) (`REQ-RICH-01` … `REQ-RICH-07`) y cambio archivado [`2026-08-20-add-rich-classroom-posts`](openspec/changes/archive/2026-08-20-add-rich-classroom-posts/proposal.md). Conserva `body: string`, reutiliza KaTeX vendorizado, no añade dependencias ni implementación Android paralela, limita nuevas escrituras a 40.000 caracteres y no agrega consultas. `RichText` construye nodos React escapados sin `dangerouslySetInnerHTML`; KaTeX opera con `trust: false`, límites de expansión/tamaño y fallback al TeX fuente. Creación y edición comparten `RichPostEditor` con vista previa diferida; texto plano histórico sigue visible sin migración. Verificación: `verify:fast` 189/189, `verify:invariants` 31/31, ESLint limpio, OpenSpec válido, test-locking 23 archivos y `pnpm test` 214/214 tras build Next.js 16. El `format:check` global conserva 18 hallazgos preexistentes fuera del diff; todos los archivos modificados por CEO-55 pasan Prettier dirigido. Sin despliegue a producción; queda el smoke test físico Android después del preview/deploy.
