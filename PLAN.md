...
- [ACTIVE] **P5 — Capacitor migration: device verification (TASK-19) and the Gradle build are the only things left.** Branch `claude/capacitor-migration`, tag `android-legacy-v13` holds the pre-migration tree. Nothing in this migration has been compiled or run on hardware: the Windows box has no Android SDK, so `assembleDebug`/`bundleRelease` are unverified, and the §7.2 checklist (native sign-in, foreign-domain rejection, back button three levels deep, external link, notification permission granted and denied, library in airplane mode, feed scroll, PDF open) has not been run. Note that AGP moved 8.7.2 → 8.10.1 and `compileSdk`/`targetSdk` to 36 so Play's August 2026 target floor is met; that pairing has not been compiled either. `android/`, `capacitor.config.ts`
