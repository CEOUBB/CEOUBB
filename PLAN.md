...
- [ACTIVE] **P1: Publication studio, full-page authoring and cover consolidation.** Felipe Arce. Branch `feat/estudio-publicacion`. The `Materiales` tab is gone: the cover now carries every teacher publication with its attachments, and `+ Nueva publicación` / `Importar Moodle` moved to the course header. The modal wizard and composer were replaced by a full-screen studio inside the `course` screen, with presets that seed a template, an inspector for folder, due date, link, attachments and alerts, a per-section local draft, and reading statistics. The editor gained H1-H3, lists, quote, notice and assessment callouts, a thematic divider and a `/` command menu. Attachments now live in the publication document itself (`attachments`, capped at six). Contract in `openspec/specs/classroom/publication-studio/spec.md`; delta archived at `openspec/changes/archive/2026-09-01-publication-studio-full-page/`. Pending: multi-role manual pass over the studio on desktop, Android WebView and a physical device, which needs an authenticated session this harness cannot produce. `app/views/classroom/`, `lib/publication-workflow.ts`, `lib/rich-text.ts`, `lib/multimodal-editor.ts`, `lib/firebase/`
- [NEXT] **The local development database is stale and fails two integration tests.** `local.db` predates two migrations. Its `users` table still carried the retired password-login columns (`rut`, `password_salt`, `password_hash`, all `NOT NULL`), so every `INSERT INTO users` from `dev-login` died with `SQLITE_CONSTRAINT_NOTNULL: users.rut`; that table was rebuilt against `db/schema.ts` on 2026-09-01 and quick-access sign-in works again. The database is still missing `section_profiles`, so `GET /api/teacher/courses` answers 500 and no section can be created or opened locally. While that holds, `renders the isolated teacher preview with noindex and a bounded DOM` (404) and `refuses to change the rank of an account holding the owner rank` (500) keep failing in `tests/rendered-html.test.mjs`; both reproduce on a clean `main` at `2558e75` and are unrelated to any feature work. The fix is regenerating `local.db` from the current schema and seeding a minimal academic tree. `local.db` is gitignored, so production is unaffected. `db/schema.ts`, `local.db`, `scripts/seed-staging.mjs`
