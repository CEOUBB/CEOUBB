...
WshShell.Run "node scripts/discord-claude-bridge.js", 0, False
"@
...
Write-Host "✅ CEOUBB Claude Bridge configured in Windows Startup folder:" -ForegroundColor Green
Write-Host "   Target: $vbsTarget" -ForegroundColor Cyan
Write-Host "   Working Directory: $repoDir`n" -ForegroundColor DarkGray

# Stop any existing instance
Get-CimInstance Win32_Process -Filter "Name = 'node.exe'" | Where-Object { $_.CommandLine -like "*discord-claude-bridge*" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force } -ErrorAction SilentlyContinue
...
Write-Host "   Claude bot is now online and will automatically start with Windows." -ForegroundColor Yellow
